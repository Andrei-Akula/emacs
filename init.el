...
(defun split-window-sensibly-try-hor-first (&optional window)
  "Split WINDOW in a way suitable for `display-buffer'.
WINDOW defaults to the currently selected window.

Try to split widow horizontally (side by side) fist.
...
lower window.  If this can't be done either and WINDOW is the
only window on its frame, try to split WINDOW vertically
disregarding any value specified by `split-height-threshold'.  If
that succeeds, return the lower window.  Return nil otherwise.
...
(setq
 split-width-threshold 154
 split-window-preferred-function 'split-window-sensibly-try-hor-first)
